...
<style scoped>
.guest-name-shape-arch { border-radius: 50% 50% 6px 6px / 62% 62% 6px 6px; border: 1px solid transparent; box-shadow: 0 10px 30px -10px rgba(0, 0, 0, 0.5); }
.guest-name-shape-pill { border-radius: 999px; border: 1px solid transparent; box-shadow: 0 10px 30px -10px rgba(0, 0, 0, 0.5); }
.guest-name-shape-hexagon { clip-path: polygon(22% 0%, 78% 0%, 100% 50%, 78% 100%, 22% 100%, 0% 50%); box-shadow: 0 10px 30px -10px rgba(0, 0, 0, 0.5); }
.guest-name-shape-ribbon { clip-path: polygon( 0% 0%, 100% 0%, 100% 36%, 90% 50%, 100% 64%, 100% 100%, 0% 100%, 0% 64%, 10% 50%, 0% 36% ); box-shadow: 0 10px 30px -10px rgba(0, 0, 0, 0.5); }
.guest-name-box-animated { animation: guest-name-unroll 1.1s cubic-bezier(0.65, 0, 0.35, 1) both; }
@keyframes guest-name-unroll { 0% { clip-path: inset(0 50% 0 50%); opacity: 0; } 45% { opacity: 1; } 100% { clip-path: inset(0 0% 0 0%); opacity: 1; } }

.guest-name-box-custom { position: relative; width: 260px; aspect-ratio: 1.6; justify-content: center; }
.guest-name-box-custom-bg { position: absolute; inset: 0; background-size: contain; background-position: center; background-repeat: no-repeat; }

.envelope-overlay { position: absolute; inset: 0; z-index: 50; display: flex; align-items: center; justify-content: center; cursor: pointer; overflow: hidden; }

/* Classic Envelope */
.envelope-classic-wrap { position: relative; width: min(70vw, 260px); aspect-ratio: 200 / 140; filter: drop-shadow(0 15px 30px rgba(0, 0, 0, 0.4)); animation: envelope-bob 3.5s ease-in-out infinite; }
@keyframes envelope-bob { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-10px); } }
.envelope-inner-card { position: absolute; left: 50%; top: 66%; width: 58%; height: 52%; transform: translate(-50%, -35%) scale(0.5) rotate(-2deg); border-radius: 0.4rem; border: 2px solid var(--theme-accent, #e3b04a); background: linear-gradient(165deg, #fffdf7, #f3ead9); box-shadow: 0 10px 22px rgba(0, 0, 0, 0.35); display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 10%; padding: 12% 14%; opacity: 0; z-index: 4; }
.envelope-letter-icon { color: var(--theme-accent, #e3b04a); opacity: 0.9; flex-shrink: 0; }
.envelope-letter-lines { display: flex; flex-direction: column; gap: 8px; width: 100%; }
.envelope-letter-lines span { display: block; height: 6px; border-radius: 999px; background: rgba(20, 30, 45, 0.18); }
.envelope-letter-lines span:nth-child(1) { width: 90%; }
.envelope-letter-lines span:nth-child(2) { width: 65%; }

.envelope-body-svg { position: absolute; inset: 0; z-index: 2; width: 100%; height: 100%; }
.envelope-flap-svg { position: absolute; inset: 0; z-index: 3; width: 100%; height: 100%; transform-origin: 50% 13%; }

.envelope-classic-leave-active .envelope-classic-wrap { animation: none; }
.envelope-classic-leave-active .envelope-flap-svg { transform: perspective(700px) rotateX(-165deg); transition: transform 0.65s cubic-bezier(0.65, 0, 0.35, 1); }
.envelope-classic-leave-active .envelope-inner-card { animation: classic-letter-emerge 0.85s cubic-bezier(0.22, 1, 0.36, 1) 0.3s both; }
@keyframes classic-letter-emerge { 0% { transform: translate(-50%, -35%) scale(0.5) rotate(-2deg); opacity: 0; } 18% { opacity: 1; } 58% { transform: translate(-50%, -122%) scale(1.18) rotate(3deg); opacity: 1; } 100% { transform: translate(-50%, -158%) scale(1.32) rotate(-1deg); opacity: 1; } }
.envelope-classic-leave-active .envelope-body-svg { opacity: 0; transition: opacity 0.4s ease 0.55s; }
.envelope-classic-leave-active { transition: opacity 0.6s ease 0.9s; }
.envelope-classic-leave-to { opacity: 0; }

/* Wax Seal details */
.wax-seal { position: relative; width: 104px; height: 104px; transform: rotate(-3deg); filter: drop-shadow(0 10px 18px rgba(0, 0, 0, 0.5)); }
...
.wax-seal-blob { position: absolute; inset: 8%; border-radius: 42% 58% 63% 37% / 47% 41% 59% 53%; box-shadow: inset 0 3px 6px rgba(255, 255, 255, 0.35), inset 0 -8px 14px rgba(0, 0, 0, 0.4), 0 2px 4px rgba(0, 0, 0, 0.3); border: 1px solid rgba(0, 0, 0, 0.25); }
.wax-seal-ring { position: absolute; inset: 24%; border-radius: 50%; border: 1px dashed rgba(0, 0, 0, 0.25); pointer-events: none; }
.wax-seal-crack { position: absolute; inset: 0; width: 100%; height: 100%; opacity: 0; stroke: rgba(0, 0, 0, 0.6); stroke-width: 2.2; fill: none; stroke-linecap: round; stroke-linejoin: round; }
.wax-seal-initials { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; font-family: 'Cormorant Garamond', serif; font-weight: 700; font-size: 1.5rem; letter-spacing: 0.05em; color: rgba(0, 0, 0, 0.55); text-shadow: 0 1px 0 rgba(255, 255, 255, 0.25); }
.wax-crumb { position: absolute; width: 4px; height: 4px; border-radius: 50%; background: var(--theme-accent, #d4a017); opacity: 0; left: 50%; top: 50%; }
...
.split-door-leave-active .wax-seal-piece-1 { transition: transform 0.46s cubic-bezier(0.34, 1.56, 0.64, 1) 0.1s, opacity 0.32s ease 0.3s; transform: translate(20px, -22px) rotate(38deg); opacity: 0; }
.split-door-leave-active .wax-seal-piece-2 { transition: transform 0.46s cubic-bezier(0.34, 1.56, 0.64, 1) 0.14s, opacity 0.32s ease 0.34s; transform: translate(-4px, 28px) rotate(-16deg); opacity: 0; }
.split-door-leave-active .wax-seal-piece-3 { transition: transform 0.46s cubic-bezier(0.34, 1.56, 0.64, 1) 0.18s, opacity 0.32s ease 0.38s; transform: translate(-24px, -10px) rotate(-40deg); opacity: 0; }
.split-door-leave-active .wax-seal-initials { transition: opacity 0.22s ease 0.08s; opacity: 0; }
.split-door-leave-active .wax-crumb { animation: wax-crumb-fly 0.55s ease-out forwards; animation-delay: var(--delay, 0s); }
.split-door-leave-active .wax-seal-doors .door-left, .split-door-leave-active .wax-seal-doors .door-right { transition-delay: 0.55s; }

@keyframes wax-seal-press-crack { 0% { transform: rotate(-3deg) scale(1); } 30% { transform: rotate(-3deg) scale(0.88); } 55% { transform: rotate(-7deg) scale(1.08); } 80% { transform: rotate(2deg) scale(0.98); } 100% { transform: rotate(-3deg) scale(1); } }
@keyframes wax-crack-flash { 0% { opacity: 0; } 20% { opacity: 1; } 55% { opacity: 0.8; } 100% { opacity: 0; } }
@keyframes wax-crumb-fly { 0% { opacity: 0; transform: translate(0, 0) scale(1); } 15% { opacity: 1; } 100% { opacity: 0; transform: translate(var(--cx), var(--cy)) scale(0.4); } }

/* Fade & Slide Animations */
.envelope-fade-leave-active { transition: opacity 0.7s ease, transform 0.7s cubic-bezier(0.34, 1.56, 0.64, 1); }
.envelope-fade-leave-to { opacity: 0; transform: scale(1.1) translateY(-20px); }

.slide-up-open-leave-active, .slide-down-open-leave-active, .slide-left-open-leave-active, .slide-right-open-leave-active { transition: transform 0.65s cubic-bezier(0.65, 0, 0.35, 1), opacity 0.5s ease; }
...
.split-door-leave-active { transition: opacity 1.35s ease; }
.split-door-leave-active .door-left { transform: translateX(-100%); transition: transform 1.2s cubic-bezier(0.65, 0, 0.15, 1); }
.split-door-leave-active .door-right { transform: translateX(100%); transition: transform 1.2s cubic-bezier(0.65, 0, 0.15, 1); }
.split-door-leave-active .content-container { opacity: 0; transform: scale(1.08); transition: opacity 0.4s ease 0.45s, transform 0.9s cubic-bezier(0.34, 1.56, 0.64, 1); }
.split-door-leave-to { opacity: 0; }
.split-door-leave-active.style-wax-seal { transition: opacity 1.2s ease 0.6s; }
.split-door-leave-active.style-wax-seal .content-container { transition: opacity 0.25s ease 0.6s, transform 0.9s cubic-bezier(0.34, 1.56, 0.64, 1); }
...
@keyframes content-pop-away { 0% { transform: scale(1); opacity: 1; } 40% { transform: scale(1.1); opacity: 1; } 100% { transform: scale(0.85); opacity: 0; } }
@keyframes burst-flash-expand { 0% { transform: scale(1); filter: brightness(1); } 30% { transform: scale(1.05); filter: brightness(1.4); } 100% { transform: scale(1.15); filter: brightness(1); opacity: 0; } }

.confetti-particle { position: absolute; top: -8%; width: 8px; height: 14px; border-radius: 2px; opacity: 0; animation: confetti-fall 3.6s linear infinite; }
.confetti-particle:nth-child(3n) { border-radius: 50%; width: 9px; height: 9px; }
.confetti-particle:nth-child(4n) { width: 6px; height: 12px; }
@keyframes confetti-fall { 0% { transform: translate3d(0, -10px, 0) rotate(0deg); opacity: 0; } 8% { opacity: 0.95; } 88% { opacity: 0.85; } 100% { transform: translate3d(0, 560px, 0) rotate(480deg); opacity: 0; } }

.cd-1 { animation-delay: 0s; } .cd-2 { animation-delay: 0.4s; } .cd-3 { animation-delay: 0.9s; } .cd-4 { animation-delay: 1.3s; }
.cd-5 { animation-delay: 1.7s; } .cd-6 { animation-delay: 2.1s; } .cd-7 { animation-delay: 0.2s; } .cd-8 { animation-delay: 0.6s; }
.cd-9 { animation-delay: 1.1s; } .cd-10 { animation-delay: 1.5s; } .cd-11 { animation-delay: 1.9s; } .cd-12 { animation-delay: 2.3s; }

.confetti-burst-particle { position: absolute; left: 50%; top: 42%; width: 7px; height: 13px; border-radius: 2px; opacity: 0; transform: translate(-50%, -50%) scale(0.6); }
.confetti-burst-particle:nth-child(3n) { border-radius: 50%; width: 8px; height: 8px; }
.confetti-burst-leave-active .confetti-burst-particle { animation: confetti-pop 0.9s cubic-bezier(0.16, 1, 0.3, 1) var(--del, 0s) forwards; }
@keyframes confetti-pop { 0% { transform: translate(-50%, -50%) translate(0, 0) rotate(0deg) scale(0.6); opacity: 0; } 12% { opacity: 1; } 100% { transform: translate(-50%, -50%) translate(var(--bx), var(--by)) rotate(var(--brot)) scale(1); opacity: 0; } }

.modern-dark-bg { background: linear-gradient(160deg, var(--md-from), var(--md-via) 55%, var(--md-to)); }
.modern-dark-bg .md-blob { position: absolute; border-radius: 50%; filter: blur(70px); mix-blend-mode: screen; animation: md-drift 9s ease-in-out infinite; }
...
@keyframes md-drift { 0%, 100% { transform: translate(0, 0) scale(1); } 50% { transform: translate(18px, -14px) scale(1.08); } }
.modern-dark-bg .md-frame { position: absolute; inset: 22px; border: 1px solid rgba(227, 176, 74, 0.25); border-radius: 4px; pointer-events: none; }
.modern-dark-bg .md-grain { position: absolute; inset: 0; opacity: 0.05; background-image: radial-gradient(circle, #fff 1px, transparent 1px); background-size: 3px 3px; }

.minimal-light-bg { background: linear-gradient(160deg, var(--ml-from), var(--ml-via) 55%, var(--ml-to)); }
.minimal-light-bg .ml-blob { position: absolute; border-radius: 50%; filter: blur(50px); mix-blend-mode: multiply; opacity: 0.5; animation: ml-drift 10s ease-in-out infinite; }
...
@keyframes ml-drift { 0%, 100% { transform: translate(0, 0); } 50% { transform: translate(14px, 10px); } }
.minimal-light-bg .ml-dot-ring { position: absolute; inset: 20px; border-radius: 999px; border: 1px dashed rgba(0, 0, 0, 0.08); pointer-events: none; }
...
@media (prefers-reduced-motion: reduce) {
  ...
  .envelope-classic-leave-active { animation-duration: 0.2s; animation-delay: 0s; transition-duration: 0.2s; transition-delay: 0s; }
  ...
  .split-door-leave-active.style-wax-seal .content-container { animation-duration: 0.2s; animation-delay: 0s; transition-duration: 0.2s; transition-delay: 0s; }

  .confetti-burst-leave-active .content-container,
  .confetti-burst-leave-active .confetti-burst-bg,
  .confetti-burst-leave-active .confetti-burst-particle { animation: none; transition-duration: 0.2s; transition-delay: 0s; }
  .confetti-burst-leave-active { transition-duration: 0.2s; transition-delay: 0s; }
  .confetti-particle, .modern-dark-bg .md-blob, .minimal-light-bg .ml-blob { animation: none; }
  .guest-name-box-animated { animation: none; clip-path: none; opacity: 1; }
}

.cfg-bg { background: linear-gradient(160deg, var(--cfg-from) 0%, var(--cfg-via) 55%, var(--cfg-to) 100%); }
.cfg-blob { position: absolute; width: 60%; aspect-ratio: 1; border-radius: 9999px; filter: blur(60px); opacity: 0.5; background: var(--cfg-accent); animation: cfg-drift var(--cfg-duration, 900ms) ease-in-out infinite alternate; }
...
@keyframes cfg-drift { from { transform: translate(0, 0) scale(1); } to { transform: translate(8%, 10%) scale(1.15); } }
.cfg-confetti { position: absolute; top: -10%; width: 8px; height: 14px; border-radius: 2px; opacity: 0.85; animation: cfg-fall 3s linear infinite; }
