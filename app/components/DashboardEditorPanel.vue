...
// This is the real Design Studio editor - the exact same component the
// /dashboard/editor page and the /admin/wedding/[id]/editor admin page both
// render, so there's only ever one implementation to keep up to date.
// overrideWeddingId is only ever set by the admin page, so a superadmin
// edits one specific wedding instead of "whichever wedding the signed-in
// account owns"; couples hitting their own dashboard never pass it, so
// useMyWedding() falls back to its normal own-wedding lookup.
